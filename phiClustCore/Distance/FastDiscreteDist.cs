...
{
    public class FastDiscreteDist:HammingBase
    {
        ...
        public FastDiscreteDist(int binSizeG, Dictionary<string, List<double>> data):base(data,true)
        {
        ...
        /*public override int[] GetDistance(string refStructure, List<string> structures)
        {
            ...
            Dictionary<double, KeyValuePair<int, int[]>> b = null;
            //List<int> aux;
            int counter = 0;
            for (int i = 0; i < prof1.Count; i++)
            {
                ...
                {
                    var aux = b[(int)prof1[i]];

                    for (int j = 0; j < aux.Value.Length; j++)
                        locDist[aux.Value[j]]+=aux.Key;
                    //if(aux.)
                    counter++;
                }
            ...
            {
                //locDist[i] = counter + minStateDataBase[i] - locDist[i];
                int x = counter + minStateDataBase[i] - locDist[i];
                locDist[i] = 100 - (int)(100 * locDist[i]) / x;
            }
        ...
        public override int[] GetDistance(string refStructure, List<string> structures)
        {
            ...
            for (int i = 0; i < locDist.Length; i++)
                locDist[i] =counter+ minStateDataBase[i] - 2 * locDist[i];
